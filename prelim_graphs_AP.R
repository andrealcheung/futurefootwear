...
# More stats, making modifications to original graphs, trying to get them ready for faculty review
...
### Grams lost versus steps taken for measured mass loss: "grams_steps_scatter"

# Running a regression and sticking with `steps to miles`

grams_steps_lm <- lm(measured_mass_change ~ steps, data = full_data_joined)
...
grams_steps_scatter_lm <- ggplot(full_data_joined, aes(x=`steps to miles`, y=measured_mass_change))+
  geom_point()+
  theme_bw()+
  labs(x = "Distance Travelled (miles)", y = "Measured Mass Change (g)")+
...
grams_miles_lm <- lm(measured_mass_change ~ `steps to miles`, data = full_data_joined)
summary(grams_miles_lm)

# Coefficients:
#Estimate Std. Error t value Pr(>|t|)    
#(Intercept)      -1.215380   0.272539  -4.459 2.02e-05 ***
#`steps to miles` -0.004066   0.001425  -2.852   0.0052 ** 
...
tread <- tread_mass_joined %>% 
  ...
  mutate("weight_kg"= weight*0.453592) %>% #1lb = 0.453592kg
...
# Ran a regression between mass lost and tread depth change, P < 0.001, R-squared = 0.1617. 
...
# P-value is 0.3, so no significant difference in means. No Tukey's HSD
...
# P-value is 0.609, so no significant difference in means. No Tukey's HSD
...
# P-value is 0.0738, so no significant difference in means. No Tukey's HSD
...
# P-value is 0.134, so no significant difference in means. No Tukey's HSD
...
# P-value is 0.555, so no significant difference in means. No Tukey's HSD
...
# P-value is 0.98, so no significant difference in means. No Tukey's HSD
...
########################### Finding average mass grams lost per mile for entire population using directly measured mass loss ####
