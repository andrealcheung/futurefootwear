...
library(here) # organize data files nicely and helps R find them

biweeklydata <- read_csv("Bi-Weekly Data_FINAL.csv") # edit file name to match the downloaded file, updated 1/6
...
washing_test <- read_csv("WashingTestError.csv") # washing test pre and post measurements
...
clean_presurvey <- presurvey_data  %>%
  clean_names(.) %>% 
...
clean_mass <- mass_data %>% 
  ...
  mutate("measured_mass_change"= post-pre) %>% 
...
full_data_joined <- full_join(mass_data_joined, clean_shoedeets) %>%  #join all pre and post mass data, participant age/weight/name, shoe model/rubber/abrasion
  ...
  filter(name!="0") %>% 
  select(-c('Shoe ID')) %>% 
  filter(name != "BRI WINKLER") %>% #removing these individuals from our data set due to lack of data reporting. 
  ...
  mutate("measuremass_lost_per_mile"= measured_mass_change/`steps to miles`) #use steps to miles, assuming the average steps per mile is 2000



step_calculations <- full_data_joined %>% 
  mutate("milesteps"= steps/2000) %>% 
  mutate("g_per_milesteps"= measured_mass_change/milesteps) %>% 
  mutate("weight_kg"= weight*0.453592) %>% 
  mutate("g_per_milesteps_per_kg"=g_per_milesteps/weight_kg) %>% 
  mutate("g_per_step"=measured_mass_change/steps) %>% 
  mutate("g_per_10kstep"=g_per_step*10000)

#compare steps to miles and choose "best"? confirmed milesteps and `steps to miles` is the same
...
# steps per person - shows steps per person in a graph
...
grams_per_shoe_hist <- ggplot(step_calculations, aes(x=measured_mass_change))+
  geom_histogram()+
  scale_x_continuous()+
  scale_y_continuous()+
...
grams_steps_scatter <- ggplot(full_data_joined, aes(x=steps, y=measured_mass_change))+
  geom_point()+
  theme_bw()
...
mass_error<- full_data_joined %>% 
  filter(name=="CONTROL") %>% 
  group_by(model) %>% 
  summarize(min_value=min(measured_mass_change),
            max_value=max(measured_mass_change),
            avg=mean(measured_mass_change))
...
# clean up tread depth measurements: find average of the three measurements, summarize by model and location, make some prelim graphs
#note: missing L08 and L72 (Max and Cesar L final tread depth measurements - reason: unknown)
...
tread_joined <- full_join(tread_initial, tread_final) %>% 
  ...
  mutate("tread_mass_loss"=final_initial*0.1*specific_gravity*shoe_sa_cm2)
...
tread_joined_shoeID <- tread_joined %>% 
  ...
  summarize("avg_depth_change"= mean(final_initial),
            "avg_mass_change"=mean(tread_mass_loss)) 
...
tread_error <- tread_joined %>%
  ...
  filter(name=="CONTROL") %>% 
  group_by(model) %>% 
  summarize("avg_error"=mean(tread_mass_loss)) %>% 
  drop_na()
